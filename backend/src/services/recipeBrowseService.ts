...
import { jsonStringArray } from "./recipeRouteHelpers.js";
import {
...
/** My Recipes listing with optional sensory profile filtering. */
export async function browseRecipes(userId: string | null, query: BrowseQuery) {
  ...
  const cards = recipes
    ...
    .filter(
      (card) => query.filter === "showAll" || (!card.hasDietaryConflict && !card.hasSensoryConflict),
    );
